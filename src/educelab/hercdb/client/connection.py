...
class RESTConnection:
    ...
    def get_human_readable_name(self, pherc, cornice=None, pezzo=None):
        res = requests.get('whatever/we/need')
        # parse result here
        pherc_n, corn_n, pezzo_n = res
...
def connect(uri=None, user=None, password=None) -> RESTConnection:
    ...
    # open new connection
    db = RESTConnection(uri, user, password)
    return db
