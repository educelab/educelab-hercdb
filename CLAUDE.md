...
│       ├── sample_uuid_check.py
│       └── cleanup_pipelines.py
│
...
- Key enums: `DatasetType` (FlatbedScan, PGSRaw, SpectralRaw, PGSProcessed, SpectralProcessed, Registered, WebProcessed)
...
- The 2023 → 2026 reconciliation (78 backfilled sample uuids, 147 unlinked calibration/test scans, 1 dropped `sample uuid 2`) and the current spectral ground-truth file (`input_data/spectral_datasets_20260609_final.csv`, which supersedes the earlier `_reconciled.csv` with hand-corrected sample-uuid links) are recorded in `docs/data_review_notes.md` §5.
...
- A dataset counts as **complete** only when `complete == "True"` AND `missing_files == 0 AND zero_byte_files == 0 AND short_files == 0 AND bad_format_files == 0` (see `_is_fully_complete`). This is stricter than the raw `complete` flag. The DB layer applies the **same** rule on its `newest_completed` path via the shared `_FULLY_COMPLETE_PREDICATE` in `db/connection.py` (reached through `_dataset_sources_cypher`, so `find_datasets_for_educelabid` / `find_datasets_for_educelabid_with_predecessors` / `find_all_datasets_for_pherc` all get it; the work-list queries use the `AND`-prefixed `_FULLY_COMPLETE_CYPHER` form of the same string), so the report and the REST `newest_completed` results agree. Note the rule applies to **raw** datasets only — a processed dataset has no file counts, and counts as complete when its Process finished. The two are twins — one Python (`_is_fully_complete`), one Cypher (`_FULLY_COMPLETE_PREDICATE`) — and must stay in sync; `coalesce(...,0)` keeps legacy nodes without count columns behaving as the old flag-only filter.
...
- `Process -[:STAGE_OF]-> Pipeline`, `Pipeline -[:FOR]-> EduceLabID`, `Process -[:OUTPUT]-> (PGSProcessed|SpectralProcessed|Registered|WebProcessed)`
...
2. **Dataset queries** (both REST dataset endpoints pool the `[:REPLACES*0..]` chain and tag each dataset with `belongs_to_uuid`):
   - **Two dataset sources, one result.** Raw scans hang off an EduceLabID via `BELONGS_TO`; processed datasets do not — they are Process outputs, reached via `EduceLabID <-[:FOR]- Pipeline <-[:STAGE_OF]- Process -[:OUTPUT]-> d`. `_dataset_sources_cypher(anchor)` emits both branches as one scoped `CALL (anchor) { ... UNION ... }` (needs **Neo4j 5.23+**; production runs 5.26.30) with a uniform projection (`d`, `date_end`, `is_complete`, `pipeline_id`, `proc_status`), so all three methods below filter and group identically. A processed dataset is serialized with `pipeline_id` + `status` from its Process, a `date_end` taken from that Process's `end_time`, and a `complete` string (`"True"`/`"False"`, matching `normalize_complete`'s form on raw scans) derived from whether the Process finished — downstream consumers gate on `complete`, so omitting it makes processed rows read as incomplete and disappear. Raw `complete` is passed through from the CSV, never backfilled from the stricter predicate. **A row is identified as processed by its label** (`ds_type in _PROCESSED_DATASET_LABELS`), not by the presence of a Process property — the two Cypher branches are partitioned by label, so the label is the branch; gating on `pipeline_id`/`proc_status` would key off a merely-incidental field. `newest_completed` therefore means "the Process finished" for processed data and the strict file-count rule for raw scans. All four pipeline outputs are exposed (`PGSProcessed`, `SpectralProcessed`, `Registered`, `WebProcessed`), though as of 0.3.4 only PGS/SPEC stages have ever run in production, so the latter two match nothing yet.
...
6. **Work-list queries** (one family for every proc_type — do not add per-stage siblings):
   ...
   - Neo4j cannot parameterize a node label, so `_raw_candidate_rows` interpolates one looked up from `_CANDIDATE_LABELS` rather than taken from the caller. Add a proc_type by adding a dict entry, never by interpolating an argument.
...
- `GET /pherc/{pherc_id}/all-datasets` - All datasets grouped by active/assigned EduceLabID, pooled across REPLACES chains; each dataset carries `belongs_to_uuid`. Optional `dataset_type` (`FlatbedScan` | `PGSRaw` | `SpectralRaw` | `PGSProcessed` | `SpectralProcessed` | `Registered` | `WebProcessed`) / `newest_completed`.
- `GET /educelabid/{uuid}/datasets` - Datasets for a UUID, pooled across its REPLACES chain; each carries `belongs_to_uuid`. Processed outputs are included and carry `pipeline_id` + `status`. Optional `dataset_type` / `newest_completed`.
- `GET /datasets/{proc_type}/unprocessed` - Work-list for an unattended dispatcher: complete raw scans awaiting `proc_type` (`SPEC` or `PGS`, case-insensitive), one row per artifact from its newest scan. Empty list returns `200 []`, never 404; an unsupported proc_type returns 400. Backed by `find_unprocessed_datasets`.
- `GET /datasets/{proc_type}/ambiguous` - Complete scans that dispatcher skips, and why (currently: assigned to more than one P.Herc., so the output has no single object directory). Backed by `find_ambiguous_datasets`.
