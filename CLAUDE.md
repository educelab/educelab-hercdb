...
- A dataset counts as **complete** only when `complete == "True"` AND `missing_files == 0 AND zero_byte_files == 0 AND short_files == 0 AND bad_format_files == 0` (see `_is_fully_complete`). This is stricter than the raw `complete` flag. The DB layer now applies the **same** rule on its `newest_completed` path via the shared `_FULLY_COMPLETE_CYPHER` predicate in `db/connection.py` (used by `find_datasets_for_educelabid` / `find_datasets_for_educelabid_with_predecessors` / `find_all_datasets_for_pherc`), so the report and the REST `newest_completed` results agree. The two are twins — one Python (`_is_fully_complete`), one Cypher (`_FULLY_COMPLETE_CYPHER`) — and must stay in sync; `coalesce(...,0)` keeps legacy nodes without count columns behaving as the old flag-only filter.
