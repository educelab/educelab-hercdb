...
- Inputs are now matched **across pipelines but within the pipeline's artifact**. A processed dataset reaches its EduceLabID through its own Process and Pipeline, so REG/WEB scope the input on that EduceLabID rather than on the consuming pipeline — cross-pipeline inputs resolve while cross-artifact ones do not. This is the same shape as the 0.2.2 change that made PGS/SPEC match their raw scan across the EduceLabID's `REPLACES` chain: the input's identity does not depend on which pipeline is consuming it, only on which artifact it belongs to. Emitted by one helper, `_upstream_input_cypher`, shared by REG's two inputs and WEB's one. The input `MATCH` is still required, so a genuinely missing input continues to return `None` rather than recording a Process with nothing feeding it. (0.2.2 recorded that REG/WEB "match this pipeline's own output nodes and were unaffected" — that held only because no registration-only submission had been made yet.)
- Matching on `path` alone was not enough: `path` is the key those nodes are MERGEd on, but it carries no uniqueness constraint anywhere in the repo, so a mistyped or copy-pasted path resolving under a different EduceLabID would have been wired in silently — and `get_pipeline_status` and `_dataset_sources_cypher` would then report another artifact's data as this artifact's provenance, with no error raised anywhere. The artifact scoping above closes that. Note the asymmetry it introduces: PGS/SPEC walk `REPLACES` **directed** (`(e)-[:REPLACES*0..]->`), while `_upstream_input_cypher` walks it **undirected**, so a UUID replacement on either side of the chain still matches.
- `PhercGraphDatabaseLoader.add_image_processing_node` now creates the `(:Pipeline)-[:FOR]->(:EduceLabID)` edge it was missing. It MERGEd a Pipeline without linking it to the artifact, so that write path could mint a processed dataset unreachable from any artifact — invisible both to `_dataset_sources_cypher` and to the new input scoping. No production data was in that state, but nothing enforced it.
